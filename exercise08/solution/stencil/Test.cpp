...
std::vector<double> seqBounds1D;


//First and last element is overwritten with boundaries
std::vector<double> parIn1D;
std::vector<double> parOut1D;
std::vector<double> parBounds1D;
...
TEST(OneDim, Parallel) {
    ...
    jacobi1DPar(parBounds1D,epsilon, 4,&parIn1D,&parOut1D, rank, size, ghosts);
    for(int i = 0; i< 6; i++){
		EXPECT_NEAR(parOut1D[i], check[i], 0.2);
	}
}
...
TEST(TwoDim, Parallel) {
    ...
    jacobi2DPar(parBounds2D,epsilon,18,&parIn2D,&parOut2D, rank, size, ghosts);
    
	for(int i = 0; i< 16; i++){
		EXPECT_NEAR(parOut2D[i], check[i], 0.4);
	}
}
*/
...
TEST(ThreeDim, Parallel) {
    ...
    jacobi3DPar(parBounds3D,epsilon,4,&parIn3D,&parOut3D, rank, size, ghosts);
    
    
    for(int i = 0; i< 64; i++){
		EXPECT_NEAR(parOut3D[i], check[i], 0.2);
	}
}
*/
